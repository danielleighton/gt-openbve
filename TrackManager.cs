...
internal class TrackManager
{
    ...
    private static readonly TrackManager instance = new TrackManager();
    ...
    #region Track Elements


    //The cant of a railway track(also referred to as superelevation) or a road(sometimes referred to as cross slope or camber) is the difference in elevation(height) between the two edges.
    //This is normally done where the railway or road is curved; raising the outer rail or the outer edge of the road providing a banked turn, thus allowing vehicles to maneuver through the 
    //curve at higher speeds than would otherwise be possible if the surface was flat or level.
    ...
    internal struct TrackElement
    {
        ...
        internal TrackElement(double StartingTrackPosition)
        {
            ...
            this.WorldPosition = new Vector3(0.0f, 0.0f, 0.0f);
            this.WorldDirection = new Vector3(0.0f, 0.0f, 1.0f);
    ...
    //// station start
    //internal class StationStartEvent
    //    : GeneralEvent
    //{
    ...
    //internal static void JumpTrackFollower(ref TrackFollower Follower, int NewTrackJump, bool UpdateWorldCoordinates, bool AddTrackInaccurary)
    //{
    //    Follower.WorldPosition.x = (float)(CurrentTrack.Elements[NewTrackJump].WorldPosition.x);
    //    Follower.WorldPosition.y = (float)(CurrentTrack.Elements[NewTrackJump].WorldPosition.y);
    //    Follower.WorldPosition.z = (float)(CurrentTrack.Elements[NewTrackJump].WorldPosition.z);
    //}
    ...
    internal static void UpdateTrackFollower(ref TrackFollower Follower, double NewTrackPosition, bool UpdateWorldCoordinates, bool AddTrackInaccurary)
    {
        ...
        float da = (float) (Follower.TrackPosition - CurrentTrack.Elements[i].StartingTrackPosition);
        float db = (float) (NewTrackPosition - CurrentTrack.Elements[i].StartingTrackPosition);
        // track
        if (UpdateWorldCoordinates)
        {
            if (db != 0.0)
            {
                ...
                {
                    // curve
                    float r = (float)CurrentTrack.Elements[i].CurveRadius;
                    float p = (float)(CurrentTrack.Elements[i].WorldDirection.y / Math.Sqrt(CurrentTrack.Elements[i].WorldDirection.x * CurrentTrack.Elements[i].WorldDirection.x + CurrentTrack.Elements[i].WorldDirection.z * CurrentTrack.Elements[i].WorldDirection.z));
                    float s = db / Mathf.Sqrt(1.0f + p * p);
                    float h = s * p;
                    float b = s / Mathf.Abs(r);
                    float f = 2.0f * r * r * (1.0f - Mathf.Cos(b));
                    float c = Mathf.Sign(db) * Mathf.Sqrt(f >= 0.0f ? f : 0.0f);
                    float a = 0.5f * Mathf.Sign(r) * b;
                    Vector3 D = new Vector3(CurrentTrack.Elements[i].WorldDirection.x, 0.0f, CurrentTrack.Elements[i].WorldDirection.z);
                    Calc.Normalize(ref D.x, ref D.y, ref D.z);
                    float cosa = Mathf.Cos(a);
                    float sina = Mathf.Sin(a);
                    Calc.Rotate(ref D.x, ref D.y, ref D.z, 0.0f, 1.0f, 0.0f, cosa, sina);
                    Follower.WorldPosition.x = (float)(CurrentTrack.Elements[i].WorldPosition.x + c * D.x);
                    Follower.WorldPosition.y = (float)(CurrentTrack.Elements[i].WorldPosition.y + h);
                    Follower.WorldPosition.z = (float)(CurrentTrack.Elements[i].WorldPosition.z + c * D.z);
                    Calc.Rotate(ref D.x, ref D.y, ref D.z, 0.0f, 1.0f, 0.0f, cosa, sina);
                    Follower.WorldDirection.x = D.x;
                    Follower.WorldDirection.y = (float)p;
                ...
                {
                    // straight
                    Follower.WorldPosition.x = (float)(CurrentTrack.Elements[i].WorldPosition.x + db * CurrentTrack.Elements[i].WorldDirection.x);
                    Follower.WorldPosition.y = (float)(CurrentTrack.Elements[i].WorldPosition.y + db * CurrentTrack.Elements[i].WorldDirection.y);
                    Follower.WorldPosition.z = (float)(CurrentTrack.Elements[i].WorldPosition.z + db * CurrentTrack.Elements[i].WorldDirection.z);
        ...
        if (AddTrackInaccurary)
        {
            ...
            Follower.WorldPosition.x += (float)(x * Follower.WorldSide.x + y * Follower.WorldUp.x);
            Follower.WorldPosition.y += (float)(x * Follower.WorldSide.y + y * Follower.WorldUp.y);
            Follower.WorldPosition.z += (float)(x * Follower.WorldSide.z + y * Follower.WorldUp.z);
